...
void StartWindow::init_window() {
    ...
    start_local_game_button.set_position(sf::Vector2f(
        (window.getSize().x - 2 * start_local_game_button.get_shape().getSize().x) / 2.0 - 20,
        (window.getSize().y - start_local_game_button.get_shape().getSize().y) / 2.0));
    ...
    start_online_game_button.set_position(sf::Vector2f(
        (window.getSize().x - 2 * start_online_game_button.get_shape().getSize().x) / 2.0 + 20 +
            start_online_game_button.get_shape().getSize().x,
        (window.getSize().y - start_online_game_button.get_shape().getSize().y) / 2.0));
}
                            ...
                            if (start_button.is_clicked(sf::Mouse::getPosition(window))) {
                                if (number_of_players_field.get_text() != "" &&
                                    ...
                                    settings = Settings("local",
                                                      ...
                                                      std::stoi(number_of_cards_field.get_text()),
                                                      std::stoi(seconds_for_turn_field.get_text()));
                                    number_of_players = std::stoi(number_of_players_field.get_text());
                                }
                            }else{
                                int cnt  = 0;
...
void StartLocalGameWindow::init_window() {
    if(number_of_players == 0){
        ...
        number_of_players_field.set_additional_text("Enter number of players (2-4):");
        number_of_players_field.set_position(
            sf::Vector2f((window.getSize().x - 3 * number_of_players_field.get_shape().getSize().x) / 2.0 - 40,
                         (window.getSize().y - number_of_players_field.get_shape().getSize().y) / 2.0));
        ...
        seconds_for_turn_field.set_position(
            sf::Vector2f((window.getSize().x - seconds_for_turn_field.get_shape().getSize().x) / 2.0 +
                             seconds_for_turn_field.get_shape().getSize().x + 40,
                         (window.getSize().y - seconds_for_turn_field.get_shape().getSize().y) / 2.0));

    }else{
        players_names[0] = TextField(20, font, "Rick");
        players_names[0].set_additional_text("Enter first player name:");
        players_names[0].set_position(
            sf::Vector2f((window.getSize().x - 2 * players_names[0].get_shape().getSize().x) / 2.0 - 20,
                         (window.getSize().y - players_names[0].get_shape().getSize().y) / 2.0));
        players_names[1] = TextField(20, font, "Rick");
        players_names[1].set_additional_text("Enter second player name:");
        players_names[1].set_position(
            sf::Vector2f((window.getSize().x - 2 * players_names[1].get_shape().getSize().x) / 2.0 - 20,
                         (window.getSize().y - players_names[1].get_shape().getSize().y) / 2.0));
    }
